...
export function AddRegionModal({ isOpen, onClose }: AddRegionModalProps) {
    ...
    const handleSelectLocation = (location: { display_name: string; latitude: number; longitude: number }) => {
        setForm({
            ...form,
            name: location.display_name || searchQuery,
            ...
            >
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                    ...
                    <Field label={<span className="flex items-center gap-1.5"><Search size={14} />Пошук населеного пункту</span>}>
                        ...
                        {searchResults && searchResults.results.length > 0 && (
                            <div className="mt-2 bg-neutral-800 border border-neutral-700 rounded-lg max-h-48 overflow-y-auto">
                                {searchResults.results.map((loc, idx) => (
                                    ...
                                    >
                                        <div className="text-white text-sm font-medium">{loc.display_name}</div>
                        ...
                        {searchResults && searchResults.results.length === 0 && (
                            <p className="mt-2 text-sm text-gray-400">Нічого не знайдено</p>
                        )}
                    </Field>
                    ...
                    <div className="flex gap-3 pt-2">
                        ...
                        <button
                            type="submit"
                            disabled={isCreating}
                            className="flex-1 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 rounded-lg text-sm text-white font-medium transition disabled:opacity-50"
                        >
